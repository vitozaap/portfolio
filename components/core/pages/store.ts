...
const defaultPage: Item = { name: "README.md", page: "README", icon: Note01Icon }

export const usePagesStore = create<PagesStore & { actions: PagesActions }>((set) => ({
    currentPage: defaultPage,
    openedPages: [defaultPage],
    actions: {
        changePage: (page) => set((state) => {

            if (state.openedPages.some(p => p.page === page.page)) {
                return state.currentPage.page === page.page ? state : { currentPage: page }
            }
